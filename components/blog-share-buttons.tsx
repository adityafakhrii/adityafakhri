...
const socials = [
    ...
    },
    {
        ...
        icon: (
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
                <path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0h-.056zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.064-1.225-.461-1.9-.903-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
            </svg>
        ),
        url: (link: string, title: string) =>
    ...
    },
    {
        ...
        // Instagram doesn't have a direct share URL, so we copy to clipboard for sharing via Stories/DM
        url: null,
    },
]
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground hover:text-foreground">
                    ...
                    <span className="text-xs">{lang === "id" ? "Bagikan" : "Share"}</span>
                </Button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-2" align="end" sideOffset={8}>
                <div className="flex flex-col gap-0.5">
                    <p className="px-2 py-1.5 text-xs font-medium text-muted-foreground">
                        {lang === "id" ? "Bagikan ke" : "Share to"}
                    </p>

                    {socials.map((s) => {
                        if (s.url === null) {
                            // Instagram — copy link for sharing via Stories/DM
                            return (
                                <button
                                    key={s.name}
                                    onClick={handleInstagram}
                                    className="flex items-center gap-2.5 rounded-md px-2 py-2 text-sm transition-colors hover:bg-accent w-full text-left"
                                >
